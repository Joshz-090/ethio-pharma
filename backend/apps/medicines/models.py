...
class Review(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    medicine = models.ForeignKey(Medicine, on_delete=models.CASCADE, related_name='reviews', null=True, blank=True)
    pharmacy = models.ForeignKey(Pharmacy, on_delete=models.CASCADE, related_name='reviews', null=True, blank=True)
    ...
    def __str__(self):
        target = self.medicine.name if self.medicine else (self.pharmacy.name if self.pharmacy else "Unknown")
        return f"{self.rating} stars for {target} by {self.user.email}"
